...
int EventManager::processEvent()
{
    ...
    if ( mHighPriorityQueue.popEvent( &eventCode, &device, &param ) )
    {
        handledCount = mListeners.sendEvent( eventCode, device, param );

        EVTMGR_DEBUG_PRINT( "processEvent() hi-pri event " )
        EVTMGR_DEBUG_PRINT( eventCode )
        EVTMGR_DEBUG_PRINT( ", " )
        EVTMGR_DEBUG_PRINT( device )
        EVTMGR_DEBUG_PRINT( ", " )
    ...
    if ( !handledCount && mLowPriorityQueue.popEvent( &eventCode, &device, &param ) )
    {
        handledCount = mListeners.sendEvent( eventCode, device, param );
...
int EventManager::processAllEvents()
{
    ...
    while ( mHighPriorityQueue.popEvent( &eventCode, &device, &param ) )
    {
        handledCount += mListeners.sendEvent( eventCode, device, param );
    ...
    while ( mLowPriorityQueue.popEvent( &eventCode, &device, &param ) )
    {
        handledCount += mListeners.sendEvent( eventCode, device, param );
...
int EventManager::ListenerList::sendEvent( int eventCode, int device, int param )
{
    EVTMGR_DEBUG_PRINT( "sendEvent() enter " )
    EVTMGR_DEBUG_PRINT( eventCode )
    EVTMGR_DEBUG_PRINT( ", " )
    EVTMGR_DEBUG_PRINTLN( device )
    EVTMGR_DEBUG_PRINT( ", " )
    ...
    for ( int i = 0; i < mNumListeners; i++ )
    {
        ...
        {
            handlerCount++;
            (*mListeners[ i ].callback)( eventCode, device, param );
        }
    }
    ...
    if ( !handlerCount )
    {
        ...
        {
            handlerCount++;
            (*mDefaultCallback)( eventCode, device, param );
...
mNumEvents( 0 )
{
    for ( int i = 0; i < kEventQueueSize; i++ )
    {
        ...
        mEventQueue[i].device = 0;
    }
}



boolean EventManager::EventQueue::queueEvent( int eventCode, int eventDevice, int eventParam )
{
    /*
    ...
    */
    ...
    if ( !isFull() )
    {
        ...
        mEventQueue[ mEventQueueTail ].device = eventDevice;
...
boolean EventManager::EventQueue::popEvent( int* eventCode, int* eventDevice, int* eventParam )
{
    /*
    ...
    *eventDevice  = mEventQueue[ mEventQueueHead ].device;
